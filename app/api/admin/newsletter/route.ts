...
/** Verify the dan_admin cookie — same logic as proxy.ts */
...
function buildHtml(subject: string, body: string): string {
  const bodyHtml = body
    ...
    .map((line) =>
      line
        ? `<p style="margin:0 0 14px;color:#ccc;font-size:15px;line-height:1.7;">${line}</p>`
        : "<br/>",
    )
    .join("");
    ...
    <tr>
      <td align="center" style="padding:40px 16px;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;background:#080808;border:1px solid rgba(255,255,0,0.2);border-radius:16px;overflow:hidden;">
          ...
          <tr>
            <td style="padding:16px 40px 36px;">${bodyHtml}</td>
          </tr>
          <tr>
  ...
  try {
    ...
    for (let i = 0; i < validEmails.length; i += BATCH_SIZE) {
      ...
      await resend.batch.send(
        batch.map((to) => ({ from: FROM, to, subject: subject.trim(), html, text })),
      );
      sent += batch.length;
    }
