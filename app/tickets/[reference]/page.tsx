...
export default function TicketPage() {
  ...
  if (error || !ticket) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-black gap-6 px-6 text-center">
        ...
        <Link
          href="/event"
          className="px-8 py-3 rounded-full border-2 border-[#FFFF00] text-[#FFFF00] text-sm font-bold uppercase tracking-widest"
        >
          Back to Events
        </Link>
      </div>
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center px-4 pt-10">
      {/* Glow background */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse 70% 50% at 50% 40%, rgba(0,255,65,0.04) 0%, transparent 70%)",
        }}
      />
      ...
      >
        ...
        >
          ...
          {/* Divider */}
          <div
            className="relative mx-7 border-t border-dashed"
            style={{ borderColor: `${statusColor}25` }}
          >
            <div
              className="absolute -left-10 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-black border"
              style={{ borderColor: `${statusColor}30` }}
            />
            <div
              className="absolute -right-10 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-black border"
              style={{ borderColor: `${statusColor}30` }}
            />
          </div>
          ...
          <div className="px-7 pt-5 pb-4 space-y-3">
            {[
              ...
              >
                ...
                <span className="text-gray-200 text-sm font-medium truncate ml-4 max-w-[55%] text-right">
                  {row.value}
                </span>
              </div>
            ))}
          ...
          <div className="flex flex-col items-center py-7 gap-3">
            <QRCode
              value={`${process.env.NEXT_PUBLIC_APP_URL ?? ""}/admin/confirm?ref=${ticket.reference}`}
              size={180}
            />
            <p className="text-gray-700 text-[10px] tracking-widest uppercase">
              Scan to verify at the gate
            </p>
          </div>
          ...
          {sponsors.length > 0 && (
            <div
              className="px-7 pb-5 border-t border-dashed"
              style={{ borderColor: `${statusColor}15` }}
            >
              <p className="text-gray-700 text-[9px] uppercase tracking-widest text-center mt-4 mb-3">
                Sponsored By
              </p>
